(function () {
  ace.define("ace/mode/worldscript_highlight_rules", [
    "require",
    "exports",
    "module",
    "ace/lib/oop",
    ...
    var WorldscriptHighlightRules = function () {
      var keywords = "end|if|return|then|goto|or|and|not";
      var builtinConstants = "true|false|nil|_G|_VERSION";
      var functions =
        "Savemap|Player|System|Entity|Entities|Fields|Special|Memory|Point|Window|Sound|Camera|bit|byte|word";
    ...
    oop.inherits(WorldscriptHighlightRules, TextHighlightRules);
    exports.WorldscriptHighlightRules = WorldscriptHighlightRules;
  });

  ace.define("ace/mode/folding/worldscript", [
    "require",
    "exports",
    "module",
    "ace/lib/oop",
    ...
    (function () {
      ...
      this.getFoldWidgetRange = function (session, foldStyle, row) {
        ...
        if (match) {
          if (match[1]) return this.worldscriptBlock(session, row, match.index + 1);
        ...
        if (match) {
          if (match[0] === "end") {
            if (session.getTokenAt(row, match.index + 1).type === "keyword")
              return this.worldscriptBlock(session, row, match.index + 1);
          }
          if (match[0][0] === "]") return session.getCommentFoldRange(row, match.index + 1);
          return this.closingBracketBlock(session, "}", row, match.index + match[0].length);
        }
      };
      this.worldscriptBlock = function (session, row, column, tokenRange) {
  ...
  ace.define("ace/mode/worldscript", [
    "require",
    "exports",
    "module",
    "ace/lib/oop",
    "ace/mode/text",
    "ace/mode/worldscript_highlight_rules",
    "ace/mode/folding/worldscript",
    "ace/range",
    ...
    var WorldscriptHighlightRules = require("./worldscript_highlight_rules").WorldscriptHighlightRules;
    var WorldscriptFoldMode = require("./folding/worldscript").FoldMode;
    var Range = require("../range").Range;
    var WorkerClient = require("../worker/worker_client").WorkerClient;
    var Mode = function () {
      this.HighlightRules = WorldscriptHighlightRules;
      this.foldingRules = new WorldscriptFoldMode();
    ...
    (function () {
      ...
      this.getMatching = function (session, row, column) {
        ...
        if (startToken && startToken.value in indentKeywords)
          return this.foldingRules.worldscriptBlock(session, row, column, true);
      };
      ...
      this.createWorker = function (session) {
        var worker = new WorkerClient(["ace"], "ace/mode/worldscript_worker", "Worker");
      ...
      this.$id = "ace/mode/worldscript";
      this.snippetFileId = "ace/snippets/worldscript";
    }).call(Mode.prototype);
    exports.Mode = Mode;
  });
})();
